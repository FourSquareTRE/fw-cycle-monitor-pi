...
# SMB share credentials for the FWCycle network share. Never hardcode these — resolve
# them (in order) from: environment variables, an optional gitignored creds file
# (scripts/smb_credentials, KEY=VALUE), or an interactive prompt.
SMB_HOST_SHARE="//192.168.0.249/Apps/FWCycle"
SMB_USER="${SMB_USER:-Operation1}"
if [[ -z "${SMB_PASS:-}" && -f "${SCRIPT_DIR}/smb_credentials" ]]; then
    # shellcheck disable=SC1091
    source "${SCRIPT_DIR}/smb_credentials"   # may set SMB_USER and/or SMB_PASS
fi
if [[ -z "${SMB_PASS:-}" ]]; then
    read -rsp "SMB password for ${SMB_HOST_SHARE} (user ${SMB_USER}): " SMB_PASS; echo
fi
FSTAB_LINE="${SMB_HOST_SHARE} ${MOUNT_POINT} cifs _netdev,user=${SMB_USER},password=${SMB_PASS},uid=${INSTALL_USER},gid=${INSTALL_GROUP},file_mode=0775,dir_mode=0775,noperm,vers=3.0 0 0"
