...
3. Run the installer with `sudo`:
   ...
   - Adds the network share `//192.168.0.249/Apps/FWCycle` to `/etc/fstab`, mounting it at `${HOME}/FWCycle`. Share credentials are **not** hardcoded — provide them via the `SMB_USER`/`SMB_PASS` environment variables, an optional gitignored `scripts/smb_credentials` file, or the interactive prompt the installer shows.
